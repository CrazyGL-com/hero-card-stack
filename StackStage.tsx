...
export default function StackStage(props: StageProps) {
	...
	// Hold the hero "not ready" until every screenshot texture has settled
	// (loaded or errored). These textures feed GL card materials — the DOM
	// can't see them — so readiness must wait on the THREE.TextureLoader
	...
	React.useEffect(() => {
		...
		let cancelled = false;
		// Gate readiness on EVERY screenshot texture. Count how many slots we
		// actually kick off a load for, then flip the gate true once they've
		// all settled (success OR error). If none load (all empty), open the
		// gate immediately so readiness can never hang.
		const total = screenshots.filter((u) => !!u).length;
		let settled = 0;
		const settleOne = () => {
			settled += 1;
			if (settled >= total) setAssetReady(true);
		};
		if (total === 0) {
			setAssetReady(true);
		}
